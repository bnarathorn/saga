...
function renderDashboard(permissions?: readonly Permission[], route?: string) {
...
describe('the Dashboard', () => {
  ...
  it('asks for the whole server, never one project, so health does not depend on a selection', async () => {
    const { calls } = stubFetch(stubs);
    renderDashboard();

    await screen.findByRole('region', { name: 'Job queue' });
    expect(calls.some((call) => call.url.includes('project_id='))).toBe(false);
  });
  ...
  it('ignores a filter value the API would reject rather than showing an error', async () => {
    ...
    renderDashboard(undefined, '/?job_type=not_a_job_type&state=queued');
  ...
  it('offers a viewer no operational action, and no audit log it may not read', async () => {
    // `GET /api/shrine/audit` requires `security:manage`. Requesting it anyway would replace an
    // absent panel with a 403 error, which is why the panel is gated rather than merely empty.
    const { calls } = stubFetch({
      ...stubs,
      '/api/shrine/jobs?limit=25': { body: { ...empty, items: [job()] } },
    });
    renderDashboard(VIEWER_PERMISSIONS);
